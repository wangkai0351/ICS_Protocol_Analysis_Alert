  ...
  *proto = iph->protocol;
  ...
  if(ret == NULL) {
	...
	return(NULL);
	  ...
	  newflow->commandlist = json_object_new_array();
      if(version == 4) {
	...
	memset(newflow->dst_id, 0, SIZEOF_ID_STRUCT);
     ...
     // printf("inflow any1:%d ",newflow->detected_protocol);
      return newflow;
    }
  } else {
    ...
    //printf("inflow any2:%d ",flow->detected_protocol);
    return flow;
  }
}
...
static unsigned int packet_processing(struct ndpi_workflow * workflow,
 ...
 if(flow->detection_completed) 
  	{
  	switch(flow->detected_protocol.protocol) {
	case NDPI_PROTOCOL_S7COMM:
	  	dissects7comm((char*)payload,payload_len,flow->commandlist);
	  	break;
    case NDPI_PROTOCOL_MODBUS_TCP:
	  	dissectmodbustcp((char*)payload,payload_len,flow->commandlist);
	  	break;
	case NDPI_PROTOCOL_DNP30:
		dissectDnp30((char*)payload,payload_len,flow->commandlist);
	  	break;
	case NDPI_PROTOCOL_IEC104:
		dissectIEC104((char*)payload,payload_len,flow->commandlist);
	  	break;
	case NDPI_PROTOCOL_SBUS:
		dissectSBUS((char*)payload,payload_len,flow->commandlist);
	    break;
	case NDPI_PROTOCOL_OMRON:
		dissectOmron((char*)payload,payload_len,flow->commandlist);
	    break;
	case NDPI_PROTOCOL_RTPS:
		dissectRTPS((char*)payload,payload_len,flow->commandlist);
		break;
 		}
 		return(0);
 	}
  ...
  flow->detected_protocol = ndpi_detection_process_packet(workflow->ndpi_struct, ndpi_flow,
   ...
   NDPI_LOG(0,workflow->ndpi_struct,NDPI_LOG_DEBUG,"flow proto %d\n",flow->detected_protocol);
   /*if(flow->detection_completed) 
 	{
