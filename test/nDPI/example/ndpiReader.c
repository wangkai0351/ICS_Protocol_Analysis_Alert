...
static void node_print_known_proto_walker(const void *node, ndpi_VISIT which, int depth, void *user_data) {
  ...
  if((which == ndpi_preorder) || (which == ndpi_leaf)) /* Avoid walking the same node multiple times */
    printFlow(thread_id, flow);
  char *rsp = json_object_to_json_string(flow->commandlist);
  printf("        commandlist:%s\n",rsp);

  //void alert_s7comm(struct flow->commandlist)
  //{
    ...
    {
      struct json_object* instance_obj = json_object_array_get_idx(flow->commandlist,i);
      char *instance_obj_rsp = json_object_to_json_string(instance_obj);
      //printf("==%s\n",instance_obj_rsp);
      struct json_object* instance_obj_command = json_object_object_get(instance_obj,"command");
      //char *instance_obj_command_rsp = json_object_to_json_string(instance_obj_command);
      //printf("command:%s\n",instance_obj_command_rsp);
      char *command_char = json_object_get_string (instance_obj_command);
      //printf("%s\n",command_char);

      if(!(strcmp(command_char, "REQUEST DOWNLOAD")&strcmp(command_char,"DOWNLOAD BLOCK")&strcmp(command_char,"DOWNLOAD ENDED")&strcmp(command_char,"PLC CONTROL")&strcmp(command_char,"PLC STOP")))
	{
          command_level[i] = 4;
	}
      else
      {
          if(!(strcmp(command_char, "CPU SERVICE")&strcmp(command_char,"WRITE VARIABLE")&strcmp(command_char,"START UPLOAD")&strcmp(command_char,"UPLOAD")&strcmp(command_char,"END UPLOAD")&strcmp(command_char,"CPU FUNCTIONS -> READ SZL")&strcmp(command_char,"BLOCK FUNCTIONS -> LIST BLOCKS")&strcmp(command_char,"BLOCK FUNCTIONS -> LIST BLOCK TYPE")&strcmp(command_char,"BLOCK FUNCTIONS -> GET BLOSK INFO")))
          {
            command_level[i]=3;
          }
          else
